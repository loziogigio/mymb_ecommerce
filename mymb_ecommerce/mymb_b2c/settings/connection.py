...
from mymb_b2c.resources import Webhook
from mymb_b2c.session import Session
...
def temp_mymb_b2c_session(func):
	"""Any function that needs to access mymb_b2c api needs this decorator. The decorator starts a temp session that's destroyed when function returns."""
	...
	def wrapper(*args, **kwargs):
		...
		if setting.is_enabled():
			auth_details = (setting.mymb_b2c_url, API_VERSION, setting.get_password("password"))
...
def register_webhooks(mymb_b2c_url: str, password: str) -> List[Webhook]:
	"""Register required webhooks with mymb_b2c and return registered webhooks."""
	new_webhooks = []

	# clear all stale webhooks matching current site url before registering new ones
	unregister_webhooks(mymb_b2c_url, password)

	with Session.temp(mymb_b2c_url, API_VERSION, password):
...
def unregister_webhooks(mymb_b2c_url: str, password: str) -> None:
	"""Unregister all webhooks from mymb_b2c that correspond to current site url."""
...
def get_callback_url() -> str:
	...
	return f"https://{url}/api/method/mymb_ecommerce.mymb_b2c.connection.store_request_data"
...
def process_request(data, event):
	...
	# create log
	log = create_mymb_b2c_log(method=EVENT_MAPPER[event], request_data=data)
...
def _validate_request(req, hmac_header):
	...
	if sig != bytes(hmac_header.encode()):
		create_mymb_b2c_log(status="Error", request_data=req.data)
