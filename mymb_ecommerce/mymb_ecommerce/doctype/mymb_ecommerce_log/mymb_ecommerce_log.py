...
class MymbEcommerceLog(Document):
	def validate(self):
		self._set_title()
	...
	def clear_old_logs(days=90):
		table = frappe.qb.DocType("Mymb Integration Log")
		frappe.db.delete(
...
def create_log(
	...
	if make_new:
		log = frappe.get_doc({"doctype": "Mymb Ecommerce Log", "integration": cstr(module_def)})
		log.insert(ignore_permissions=True)
	else:
		log = frappe.get_doc("Mymb Ecommerce Log", frappe.flags.request_id)
...
def resync(method, name, request_data):
	...
	frappe.db.set_value("Mymb Ecommerce Log", name, "status", "Queued", update_modified=False)
	frappe.db.set_value("Mymb Ecommerce Log", name, "traceback", "", update_modified=False)

	if not method.startswith("mymb_ecommerce."):
		return
