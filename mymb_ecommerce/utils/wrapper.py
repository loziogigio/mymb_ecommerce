...
def build_filter_group_list(data: JsonDict , csoci : str) -> JsonDict:
    ...
    if "facets_results" in data and "category" in data["facets_results"]:
        category_filters = data["facets_results"]["category"]

        # Normalise list-based responses that come back from the API
        raw_results = category_filters.get("results")
        if isinstance(raw_results, list):
            ...
            for entry in raw_results:
                ...
                candidate_id = entry.get("node") or entry.get("id") or entry.get("value") or entry.get("key")
                if not candidate_id:
                    path_ids = extract_category_ids(entry.get("filter", ""))
                    if path_ids:
                        candidate_id = path_ids[-1]
                    elif "label" in entry:
                        candidate_id = str(entry["label"])

                if not candidate_id:
                    continue

                normalized_results[str(candidate_id)] = entry

            category_filters["results"] = normalized_results
